...
def tf_overlap(x1, len1, x2, len2):
    len1_half = len1 / 2
    len2_half = len2 / 2
...
def tf_multi_overlap(x1, len1, x2, len2):
    len1_half = len1 / 2
    len2_half = len2 / 2
    ...
    overlap = right - left
    overlap = tf.maximum(overlap, tf.zeros_like(overlap))

    return overlap


def tf_box_intersection(a, b):
    w = tf_overlap(a[0], a[2], b[0], b[2])
    h = tf_overlap(a[1], a[3], b[1], b[3])
    area = w * h
    return area


def tf_boxes_intersection(a, b):
    w = tf_multi_overlap(a[..., 0], a[..., 2], b[..., 0], b[..., 2])
    h = tf_multi_overlap(a[..., 1], a[..., 3], b[..., 1], b[..., 3])
    area = w * h
    return area


def tf_box_union(a, b):
    i = tf_box_intersection(a, b)
    u = a[2] * a[3] + b[2] * b[3] - i
    return u


def tf_boxes_union(a, b):
    i = tf_boxes_intersection(a, b)
    u = a[..., 2] * a[..., 3] + b[..., 2] * b[..., 3] - i
    return u


def tf_box_iou(a, b):
    iou = tf_box_intersection(a, b) / tf_box_union(a, b)
    return iou


def tf_boxes_iou(a, b):
    iou = tf_boxes_intersection(a, b) / tf_boxes_union(a, b)
    return iou


def tf_anchor_iou(a, b):
    box_a = a[2:4]
    box_a = tf.concat([tf.constant([0, 0], tf.float32), box_a], 0)

    # box_a = tf.constant([0, 0, a[2], a[3]], tf.float32)
    box_b = tf.constant([0, 0, b[0], b[1]], tf.float32)
    iou = tf_box_iou(box_a, box_b)
    return iou
